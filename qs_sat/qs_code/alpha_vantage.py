...
class AlphaVantage(object):
    ...
    def get_daily_historic_data_csv(self, ticker, start_date, end_date, path='./'):
        """
        ...
        'pd.DataFrame', The frame of OHLCV prices and volumes
        """
        prices = pd.read_csv(path + ticker + '.csv',encoding='UTF-8')
        prices.loc[:,'timestamp'] = prices.loc[:,'timestamp'].apply(lambda x: pd.to_datetime(x, format='%Y-%m-%d'))
